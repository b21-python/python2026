...
groundSpeed = 4
obstacles = generateObstacles("___3_________22______1111_____4____", width, 20, 10, groundLevel)
...
while gameActive:
    ...
    if intersectsObstacles:
        ...
        if updatedX + player["Sprite"].get_width() > ob.x and updatedX < ob.right:
            updatedX = ob.x - player["Sprite"].get_width()
        if updatedY + player["Sprite"].get_height() > ob.y:
            updatedY = ob.y - player["Sprite"].get_height()
        fallSpeed = 0
